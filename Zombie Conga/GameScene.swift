...
//        debugDrawPlayableArea()
        ...
        catsLabel.position = CGPoint(x: playableRect.size.width/2 - CGFloat(200), y: -playableRect.size.height/2 + CGFloat(200))
...
//        checkCollisions()
    ...
    func spawnEnemy(){
        ...
        enemy.position = CGPoint(
            x: cameraNode.position.x + cameraRect.width/2 + enemy.size.width/2,
            y: CGFloat.random(min: playableRect.minY + enemy.size.height/2, max: playableRect.maxY - enemy.size.height/2))
        
        addChild(enemy)
        
        let actionMove = SKAction.moveTo(x: cameraRect.minX, duration: 2.0)
    ...
    func zombieHit(enemy: SKSpriteNode){
        ...
        let setHidden = SKAction.run(){[weak self] in self?.zombie.isHidden = false
    ...
    func loseCat(){
        ...
        enumerateChildNodes(withName: "train") {node, stop in
            ...
            node.name = ""
            node.run(SKAction.sequence([SKAction.group([SKAction.rotate(byAngle: π*4, duration: 1.0),
                    SKAction.move(to: randomSpot, duration: 1.0), SKAction.scale(to: 0, duration: 1.0)]),
